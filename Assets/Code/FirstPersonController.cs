...
public class FirstPersonController : MonoBehaviour {
	...
	void Update () {
		...
		if (!this.jumpLocked && this.timeInAir < 0.5f && Input.GetAxisRaw("Jump") > 0.1f && !UIManager.instance.isGameOver) {
			this.timeInAir = 0.5f;
            float pitch = Mathf.Clamp(Vector3.Angle(this.transform.forward, cameraTransform.forward), 0, 90) / 90 + 1;
			this.verticalSpeed = this.JumpStrength * pitch;
	...
	private bool onGround() {
		...
		return Physics.SphereCast(ray, this.characterController.radius, this.characterController.height * 3 / 2 - this.characterController.radius + 0.1f);
	}

    public void Retry()
    {
        this.transform.position = new Vector3(Random.Range(5, 60), Random.Range(5, 60), Random.Range(5, 40));
    }
