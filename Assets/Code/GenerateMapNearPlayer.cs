...
public class GenerateMapNearPlayer : MonoBehaviour {
	...
	private void generate() {
		...
		bool any = false;
        //print(String.Format("{0} {1}", Mathf.FloorToInt(chunkX - this.Range / chunkSize), chunkX + this.Range / chunkSize));
        //print(String.Format("{0} {1}", Mathf.FloorToInt(chunkZ - this.Range / chunkSize), chunkZ + this.Range / chunkSize));
        for (int x = Mathf.FloorToInt(chunkX - this.Range / chunkSize); x < chunkX + this.Range / chunkSize; x++) {
			...
			for (int z = Mathf.FloorToInt(chunkZ - this.Range / chunkSize); z < chunkZ + this.Range / chunkSize; z++) {
                ...
                //Debug.Log(String.Format("{0} {1}", distance, closestDistance));
